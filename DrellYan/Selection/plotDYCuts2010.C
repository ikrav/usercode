...
bool verboseEvent(const mithep::TEventInfo *info);

//=== MAIN MACRO =================================================================================================

void plotDYv2(const TString conf) 
{  
  gBenchmark->Start("plotDYv2");
...
//       eventTree->SetBranchAddress("TrackJet",   &trackJetArr);   TBranch *trackJetBr   = eventTree->GetBranch("TrackJet");
	...
	bool verbose = verboseEvent(info);
	bool verbose1 = verbose;
	verbose = false; // reduce printout
	if(verbose1)
	  printf("\nEvent of interest: run = %d   lumi = %d    event = %d  ",info->runNum,
		 info->lumiSec, info->evtNum);
...
// 	printf("debug: passed event trigger\n");
	dielectronArr->Clear(); 
	dielectronBr->GetEntry(ientry);	
	if(verbose)
	  printf("  event trigger matched, total dielectrons: %d\n", dielectronArr->GetEntriesFast());
	if(verbose1 && dielectronArr->GetEntriesFast())
	  printf(" 0 ");
// 	printf("debug: number of dielectrons: %d\n", dielectronArr->GetEntriesFast());
	// loop through dielectrons
        for(Int_t i=0; i<dielectronArr->GetEntriesFast(); i++) {
	  mithep::TDielectron *dielectron = (mithep::TDielectron*)((*dielectronArr)[i]);
// 	  printf("debug: start electron loop\n");
	  if(verbose)
	    printf("   candidate %d, mass=%f, pt1=%f, pt2=%f\n", i, 
		   dielectron->mass, dielectron->pt_1, dielectron->pt_2);
	  if(verbose1)
	    printf(" 1c ");
          // Exclude ECAL gap region
	  ...
	  if(verbose)
	    printf("    passed eta gap cut\n");
	  if(verbose1) printf(" 2 ");
// 	  printf("debug: passed eta cuts\n");
	  //
...
//    	    double corr2 = findEnergyScaleCorrection(dielectron->scEta_2);
	  ...
	  if(verbose)
	    printf("     passed Et cuts\n");
	  if(verbose1) printf(" 3 ");
// 	  printf("debug: passed Et cuts\n");
...
// 	  printf("debug: passed electron trigger match\n");
	  ...
	  if(verbose)
	    printf("    passed eta cut\n");
	  if(verbose1) printf(" 4 ");
// 	  printf("debug: passed eta < 2.5\n");
          if(!(dielectron->isEcalDriven_1)           || !(dielectron->isEcalDriven_2))           continue;  // not ECAL seeded electrons? Skip to next event...
// 	  printf("debug: passed ecal driven\n");
	  if(verbose)
	    printf("    passed ecal driven cut, coming IP 1: d0,dz and 2: d0,dz   %f %f   %f %f\n", 
		   dielectron->d0_1, dielectron->dz_1,dielectron->d0_2, dielectron->dz_2);
	  if(verbose1) printf(" 5 ");

	  // Impact parameter cuts on both electrons as for WW
	  if( fabs(dielectron->d0_1)>0.02 || fabs(dielectron->d0_2)>0.02) continue;
	  if( fabs(dielectron->dz_1)>1.0  || fabs(dielectron->dz_2)>1.0 ) continue;
	  if(verbose)
	    printf("    passed IP cuts\n");
	  if(verbose1) printf(" 6 ");

	  // Use combined isolation, as WW analysis, for WP80v2
	  if(!passWP80(dielectron,kTRUE,verbose)) continue;  
	  if(verbose)
	    printf("    passed WP80 cuts\n");
	  if(verbose1) printf(" 7 ");
	  ...
	  // event printout
          if(isam==0)
            eventDump(evtfile, dielectron, info->runNum, info->lumiSec, info->evtNum, eventTriggerBit);
               ...
               const UInt_t runNum, const UInt_t lumiSec, const UInt_t evtNum, const ULong_t trigger)
{
  ...
  if(dielectron->hltMatchBits_1 & trigger)
    ofs << " PASS" << endl; 
  else
    ofs << " FAIL" << endl;
  ...
  if(dielectron->hltMatchBits_2 & trigger)
    ofs << " PASS" << endl; 
  else
    ofs << " FAIL" << endl;
}    
...
double findEnergyScaleCorrection(double eta){
  ...
  // Old constants
// //          const Double_t scaleEB = 1.0145;
// //          const Double_t scaleEE = 1.0332;
...
// 	  }
  ...
  // Energy scale corrections form W charge asymmetry
  // for Dec 22 ReReco
  // These are for WP80 electrons
  double corr = 1.0;
  if(fabs(eta) < 0.4)
    corr = 0.993988;
    ...
    corr = 0.999162;
  else if( fabs(eta)>=1.2 && fabs(eta) < 1.5)
    corr = 1.00794;
...
867100730
  ...
  for(int i=0; i<n; i++){
    if(info->runNum == run[i] && info->evtNum == event[i] ) return true;
  }
