...
void prepareYields1D(const TString conf  = "data_plot.conf") 
{  
  gBenchmark->Start("prepareYields1D");
...
//   CPlot::sOutDir = outputDir + TString("/plots");
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
	...
	//   	double corr1 = escale.getEnergyScaleCorrection(data.scEta_1);
	//   	double corr2 = escale.getEnergyScaleCorrection(data.scEta_2);
 	double corr1 = 1;
 	double corr2 = 1;
...
//   fNameOutHists += TString::Format("%d_%d",stamp.GetDate(),stamp.GetTime());
  ...
  gBenchmark->Show("prepareYields1D");      
}
